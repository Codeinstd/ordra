...
// chart / HRIS lookup; kept simple and swappable here.
async function resolveRoleToUser(role: string, pr: PurchaseRequest): Promise<string> {
  ...
  const roleHolder = await prisma.user.findFirst({
    where: { role, department: pr.department },
  });
  const fallback = roleHolder ?? (await prisma.user.findFirst({ where: { role } }));
  if (!fallback) throw new Error(`No user found for role "${role}"`);
  return fallback.id;
}
...
function chainConditionMet(condition: string | undefined, pr: PurchaseRequest): boolean {
  if (!condition) return true;
  // Minimal expression support for the demo: "amount > 100000"
...
async function resolveCandidateChain(pr: PurchaseRequest, policy: ApprovalPolicy): Promise<ResolvedSlot[]> {
  ...
  for (const raw of template) {
    if (!chainConditionMet(raw.condition, pr)) continue;

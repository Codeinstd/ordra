...
export async function approveAndSendDraft(messageId: string, approverId: string) {
  ...
  const thread = await prisma.negotiationThread.update({
    where: { id: message.threadId },
    data: { status: "sent" },
    include: { quote: { include: { vendor: true, rfqEvent: true } } },
  });

  if (thread.quote.vendor.contactEmail) {
    await sendEmail(thread.quote.vendor.contactEmail, `Re: ${thread.quote.rfqEvent.title}`, message.body);
  } else {
    console.warn(`[negotiation] vendor ${thread.quote.vendor.id} has no contactEmail — message approved but not sent`);
  }
  return message;
}

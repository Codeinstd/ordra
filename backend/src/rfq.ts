...
// TODO: wire to a real mailer (SendGrid/Gmail API). This just records the
// send and creates the Quote placeholder each vendor's reply will attach to.
export async function sendQuoteRequest(rfqId: string, vendorId: string, subject: string, body: string) {
  console.log(`[stub email send] to vendor ${vendorId}: ${subject}\n${body}`);

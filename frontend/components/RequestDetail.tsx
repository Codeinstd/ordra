    ...
    <div className="flex-1 p-8">
      ...
      {request.auditLog && request.auditLog.length > 0 && (
        <div className="mt-8 border-t border-line pt-4">
          <button onClick={() => setShowAudit(!showAudit)} className="font-body text-sm text-slate hover:text-ink">
            {showAudit ? "Hide" : "Show"} audit trail ({request.auditLog.length})
          </button>
          {showAudit && (
            <div className="mt-3 space-y-2">
              {request.auditLog.map((e) => (
                <div key={e.id} className="flex items-baseline justify-between border-t border-line pt-2">
                  <span className="font-mono text-xs uppercase text-slate">{e.eventType.replace(/_/g, " ")}</span>
